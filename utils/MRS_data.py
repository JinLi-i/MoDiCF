...
class MRS_Data(object):
    def __init__(self, path, batch_size):
        ...
        for uid, test_items in test.items():
            ...
            try:
                ...
                self.test_len_list.append(len(test_items))
            except:
                continue
        ...
        for uid, val_items in val.items():
            ...
            try:
                ...
                self.val_len_list.append(len(val_items))
            except:
                continue
        self.pos_items_per_u = self.get_pos_items_per_u(self.test_set.keys())
        self.val_len_list = np.asarray(self.val_len_list)
        self.test_len_list = np.asarray(self.test_len_list)
    ...
    def get_pos_items_per_u(self, test_users):
        u_ids = []
        i_ids = []
        for i, u in enumerate(test_users):
            try:
                u_ls = self.train_items[u]
            except:
                u_ls = []
            i_len = len(u_ls)
            if i_len == 0:
            ...
            i_ids.extend(u_ls)
        return np.asarray([u_ids, i_ids], dtype=np.int32)